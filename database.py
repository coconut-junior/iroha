...
if not 'users.db' in files:
    ...
    c = con.cursor()
    c.execute("create table users (uid,user_name,iroha_name, last_message, last_response, key)")
else:
    con = sqlite3.connect('users.db')
    c = con.cursor()
...
def addUser(uid):
    ...
    if c.fetchone() == None:
        ...
        c.execute("insert into users values (:uid, :user_name, :iroha_name, :last_message, :last_response, :key)",
                {"uid":uid, "user_name":"dude", "iroha_name":"iroha", "last_message":"", "last_response":"","key":"none"})

    con.commit()

def updateUser(uid, user_name, iroha_name, last_message, last_response, key):
    global con
    global c
    c.execute("select * from users where uid=:uid",{"uid":uid})
    ...
    c.execute("update users set key=:key where uid=:uid",{"uid":uid,"key":key})
